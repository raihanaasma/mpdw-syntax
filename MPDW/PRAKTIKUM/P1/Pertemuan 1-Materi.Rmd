...
kalau data statisioner memakai 80% dan 20%
tapi kalau data tidak statisioner potong antara data puncak atau turun jangan dipotong di tengah
...
datanya menggunakan plot bukan plot.ts karena datanya udah time series
...
library(ggplot2)
ggplot() + 
  geom_line(data = data2, aes(x = Period, y = Sales, col = "Data Latih")) +
...
```
DATA NA itu data yang dimundurin  1 periode 
...
```
pada bagian pemulusan itu n=4 maka sama , pada data ramalan dimundurin 1 lalu data sisanya 23 data terakhir sama kayak data peramalan terakhir 
...
akurasi_train.sma
```
MAPE yang baik dibawah 10 atau 20 
...
library(ggplot2)
ggplot() + 
  geom_line(data = data1, aes(x = Periode, y = Yt, col = "Data Latih")) +
